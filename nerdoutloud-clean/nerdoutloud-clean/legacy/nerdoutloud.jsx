...
const cloud = (() => {
  ...
  const ready = () => {
    ...
    if (SUPABASE_URL && SUPABASE_ANON_KEY && typeof window !== "undefined" && window.supabase) {
      try { client = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY); } catch { client = null; }
    }
    return client;
  };
  return {
...
const TMDB_KEY = "378dfb29e72275926d9bfe3bb110678f";
...
const tmdb = {
  enabled: () => !!TMDB_KEY,
  async search(q) {
    const r = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=${TMDB_KEY}&include_adult=false&query=${encodeURIComponent(q)}`);
    if (!r.ok) throw new Error("search failed");
    const j = await r.json();
    return (j.results || []).slice(0, 8);
  },
  async filmDetails(id) {
    const r = await fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=${TMDB_KEY}&append_to_response=credits,watch/providers`);
  ...
  async discoverByService(svcName) {
    ...
    const pageUrl = (page) =>
      `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_KEY}&watch_region=US&with_watch_providers=${pid}&with_watch_monetization_types=flatrate&sort_by=popularity.desc&include_adult=false&page=${page}`;
    let all = [];
    let totalPages = 1;
    try {
...
export default function NerdOutLoud() {
  ...
  return (
    <div className="nol-root" style={{ paddingBottom: 30 }}>
      ...
      {view === "home" && <><Marquee /><FilmStrip /><WelcomeSpotlight members={members} /></>}
